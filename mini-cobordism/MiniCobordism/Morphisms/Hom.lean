...
structure CobordismHom (G : Type) where onManifolds : SmoothManifold -> G
def idCobordismHom : CobordismHom CobordismGroup where onManifolds _ := { dim := 0, generators := [] }
def swNumberHom : CobordismHom Nat where onManifolds M := stiefelWhitneyNumber M 0
def pontryaginNumberHom : CobordismHom Int where onManifolds M := pontryaginNumber M 0
