...
structure MainTheorem where
  ...
  author : String
  statement : String
def mainTheorems : List MainTheorem := [
  { name := "Pontryagin-Thom", year := 1954, author := "Thom", statement := "O_n = pi_{n+k}(MO(k))" },
  { name := "Thom class.", year := 1954, author := "Thom", statement := "N_* = F_2[x_2,x_4,...]" },
  { name := "Hirzebruch sig.", year := 1954, author := "Hirzebruch", statement := "sigma = <L(M),[M]>" },
  { name := "h-cobordism", year := 1961, author := "Smale", statement := "h-cob + dim>=5 => diffeo" }
]
