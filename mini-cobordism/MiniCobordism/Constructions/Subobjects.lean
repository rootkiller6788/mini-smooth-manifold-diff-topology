...
structure SubmanifoldData where
  ambient : SmoothManifold
  sub : SmoothManifold
  codimension : Nat
structure SurgeryData (M : SmoothManifold) where
  k : Nat
def surgeryResult (M : SmoothManifold) (data : SurgeryData M) : SmoothManifold :=
  { M with eulerChar := M.eulerChar + (if data.k % 2 = 0 then -2 else 2) }
structure Handle where
  index : Nat
  dimension : Nat
def morseTheoryStatement : String := "Every Morse function gives a handle decomposition"
def wallLGroup (n : Nat) (pi1Trivial : Bool) : String := if pi1Trivial then match n % 4 with | 0 => "Z" | 1 => "0" | 2 => "Z/2" | 3 => "0" | _ => "?" else s!"L_{n}(Z[pi_1])"
