...
theorem product_dimension (M N : SmoothManifold) : (M.product N).dim = M.dim + N.dim := rfl
theorem product_euler (M N : SmoothManifold) : (M.product N).eulerChar = M.eulerChar * N.eulerChar := rfl
theorem product_signature (M N : SmoothManifold) : (M.product N).signature = M.signature * N.signature := rfl
